...
The `generateSpaceman` function is a convenience method provided by the SpriteAI library for generating a pixel-art style spritesheet of an astronaut character. This function simplifies the process of creating a spaceman character with predefined animation states.
...
const customResult = await generateSpaceman({
  states: ['idle', 'walk', 'run', 'float', 'jetpack'],
  framesPerState: 8
});
```
