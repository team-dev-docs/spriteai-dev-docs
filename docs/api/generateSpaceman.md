...
The `generateSpaceman` function is a convenience method provided by the SpriteAI library for generating a sprite sheet of an astronaut character. This function builds upon the more general `generateCharacterSpritesheet` function, offering pre-configured options tailored for a spaceman character.
